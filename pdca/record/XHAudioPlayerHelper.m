...
- (void)playAudioWithFileName:(NSString*)fileName {
    if (fileName.length > 0) {
        ...
        if (_playingFileName && [fileName isEqualToString:_playingFileName]) {//上次播放的录音
            ...
            _timer = [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(updatePlayProcess) userInfo:nil repeats:YES];
        }
...
-(void)updatePlayProcess{
    if ([self.delegate respondsToSelector:@selector(didAudioPlayerTimer:)]) {
        [self.delegate didAudioPlayerTimer:_player];
    }
}
/*
#pragma mark - amr转wav
