...
- (void)setup {
    ...
    if (!_remind2Label) {
        UILabel *remind2Label= [[UILabel alloc] initWithFrame:CGRectMake(9.0, 98.0, 120.0, 21.0)];
        remind2Label.textColor = [UIColor whiteColor];
        remind2Label.font = [UIFont systemFontOfSize:10];
        remind2Label.layer.masksToBounds = YES;
        remind2Label.layer.cornerRadius = 4;
        remind2Label.autoresizingMask = UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleBottomMargin;
        remind2Label.backgroundColor = [UIColor clearColor];
        remind2Label.text =  @"00:00";
        remind2Label.textAlignment = NSTextAlignmentCenter;
        [self addSubview:remind2Label];
        _remind2Label = remind2Label;
    }
    if (!_remindLabel) {
